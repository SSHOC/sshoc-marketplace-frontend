...
test.describe("contact page", () => {
	test("should have document title", async ({ createContactPage }) => {
		const { i18n, contactPage } = await createContactPage();
	...
	test("should not have visible changes", async ({ createContactPage }) => {
		const { contactPage } = await createContactPage();
		await contactPage.goto();
	...
	test.describe("email", () => {
		...
		test("should send email with contact form content and display success message", async ({
			...
			const { contactPage, i18n } = await createContactPage();
			await contactPage.goto();
