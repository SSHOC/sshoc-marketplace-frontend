...
import { itemSortOrders, itemStatus } from "@/lib/data/sshoc/api/item";
import type { SourceSortOrder } from "@/lib/data/sshoc/api/source";
import type { UserSortOrder } from "@/lib/data/sshoc/api/user";
...
export function isContributedItemSortOrder(
  order: string
): order is ContributedItemSortOrder {
...
export const contributedItemsSortOrder = itemSortOrders.filter(
  isAllowedContributedItemSortOrder
);

export type ContributedItemSortOrder =
  (typeof contributedItemsSortOrder)[number];

export const defaultContributedItemsSortOrder: ContributedItemSortOrder =
  "modified-on";

export function isModerateItemSortOrder(
  order: string
): order is ModerateItemSortOrder {
...
export const moderateItemsSortOrder = itemSortOrders.filter(
  isAllowedModerateItemSortOrder
);

export type ModerateItemSortOrder = (typeof contributedItemsSortOrder)[number];

export const defaultModerateItemsSortOrder: ModerateItemSortOrder =
  "modified-on";
...
function isSupportedItemStatusQuery(status: ItemStatus): status is ItemStatus {
  ...
  const nonIndexedStatus: Array<ItemStatus> = [
    "draft",
    "deprecated",
    "disapproved",
  ];
...
export const queryableItemStatus: Array<ItemStatus> = itemStatus.filter(
  isSupportedItemStatusQuery
);
