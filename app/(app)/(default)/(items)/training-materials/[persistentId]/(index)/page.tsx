...
import { SchemaOrgMetadata } from "@/app/(app)/(default)/(items)/training-materials/[persistentId]/(index)/_components/schema-org-metadata";
import { getTrainingMaterial } from "@/app/(app)/(default)/(items)/training-materials/[persistentId]/(index)/_lib/get-training-material";
...
export default async function TrainingMaterialPage(
	...
	const breadcrumbs = [
		...
		},
		{
			...
			label: t("breadcrumbs.items.training-materials"),
		},
		{
			href: createHref({
				pathname: `/training-materials/${item.persistentId}`,
			}),
			label: item.label,
		},
	];
			...
			<section className="relative flex flex-col gap-y-8 py-16">
				<div className="grid grid-cols-[auto_1fr_auto] gap-x-6">
					...
					<CopyLinkButton
						...
						href={String(createFullUrl({ pathname: `/training-materials/${item.persistentId}` }))}
						kind="text"
					>
