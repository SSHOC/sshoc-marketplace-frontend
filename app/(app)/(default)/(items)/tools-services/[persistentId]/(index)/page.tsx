...
import { SchemaOrgMetadata } from "@/app/(app)/(default)/(items)/tools-services/[persistentId]/(index)/_components/schema-org-metadata";
import { getToolOrService } from "@/app/(app)/(default)/(items)/tools-services/[persistentId]/(index)/_lib/get-tool-or-service";
...
export default async function ToolOrServicePage(
	...
	const breadcrumbs = [
		...
		},
		{
			...
			label: t("breadcrumbs.items.tools-services"),
		},
		{
			href: createHref({
				pathname: `/tools-services/${item.persistentId}`,
			}),
			label: item.label,
		},
	];
			...
			<section className="relative flex flex-col gap-y-8 py-16">
				<div className="grid grid-cols-[auto_1fr_auto] gap-x-6">
					...
					<CopyLinkButton
						...
						href={String(createFullUrl({ pathname: `/tools-services/${item.persistentId}` }))}
						kind="text"
					>
