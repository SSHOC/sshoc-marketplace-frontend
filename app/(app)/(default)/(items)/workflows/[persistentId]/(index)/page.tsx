...
import { SchemaOrgMetadata } from "@/app/(app)/(default)/(items)/workflows/[persistentId]/(index)/_components/schema-org-metadata";
import { getWorkflow } from "@/app/(app)/(default)/(items)/workflows/[persistentId]/(index)/_lib/get-workflow";
...
export default async function WorkflowPage(props: Readonly<WorkflowPageProps>): Promise<ReactNode> {
	...
	const breadcrumbs = [
		...
		},
		{
			...
			label: t("breadcrumbs.items.workflows"),
		},
		{
			href: createHref({
				pathname: `/workflows/${item.persistentId}`,
			}),
			label: item.label,
		},
	];
			...
			<section className="relative flex flex-col gap-y-8 py-16">
				<div className="grid grid-cols-[auto_1fr_auto] gap-x-6">
					...
					<CopyLinkButton
						...
						href={String(createFullUrl({ pathname: `/workflows/${item.persistentId}` }))}
						kind="text"
					>
