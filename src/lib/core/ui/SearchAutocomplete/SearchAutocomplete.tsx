...
const SearchAutocompleteBase = forwardRef(function SearchAutocompleteBase<
  T extends object
>(
  props: SearchAutocompleteProps<T>,
  forwardedRef: ForwardedRef<HTMLDivElement>
  ...
  const layout = useListBoxLayout<T>(state, loadingState === "loadingMore");
      ...
      >
        <ListBoxBase<T>
          ...
          renderEmptyState={() => {
            ...
            return (
              <span>{t(["common", "ui", "autocomplete", "no-results"])}</span>
            );
          }}
...
interface SearchAutocompleteInputProps<T extends object>
  extends SearchAutocompleteProps<T> {
...
const SearchAutocompleteInput = forwardRef(function SearchAutocompleteInput<
  T extends object
>(
  ...
  const clearButton = (
    <ClearButton {...clearButtonProps} isDisabled={isDisabled} preventFocus />
  );
  ...
  return (
    ...
    >
      <TextFieldBase
        ...
        wrapperChildren={
          inputValue !== "" && isReadOnly !== true ? clearButton : undefined
        }
      />
    </div>
  );
