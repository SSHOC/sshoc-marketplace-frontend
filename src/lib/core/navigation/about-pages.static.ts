...
export type StaticResult = Array<{
  label: string;
...
export default async function load(): Promise<ValLoaderResult> {
  ...
  if (collection.folder == null || collection.extension == null) {
    throw new Error(
      "About pages collection config requires `folder` and `extension` to be set."
    );
  }
  ...
  return {
     ...
     */
    dependencies: [
      path.join(process.cwd(), "./src/lib/cms/collections/about-pages.ts"),
    ],
  };
}
