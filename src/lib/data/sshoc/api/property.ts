...
import { propertyTypeInputSchema } from "@/lib/data/sshoc/validation-schemas/property";
...
export function isPropertyConcept(
  property: Property
): property is PropertyConcept {
  return property.type.type === "concept";
}

export function isPropertyScalar(
  property: Property
): property is PropertyScalar {
  return property.type.type !== "concept";
}
...
export function createPropertyType(
  ...
  const json = propertyTypeInputSchema
...
export function deletePropertyType(
  ...
  const url = createUrl({
    pathname: `/api/property-types/${params.code}`,
    searchParams: params,
  });
