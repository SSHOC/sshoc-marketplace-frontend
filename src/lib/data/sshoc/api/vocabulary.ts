...
import type { PropertyTypeRef } from "@/lib/data/sshoc/api/property";
...
import { conceptInputSchema } from "@/lib/data/sshoc/validation-schemas/vocabulary";

export const conceptSortOrders = ["score", "label"] as const;

export type ConceptSortOrder = (typeof conceptSortOrders)[number];

export function isConceptSortOrder(
  sortOrder: string
): sortOrder is ConceptSortOrder {
...
export function getVocabulary(
  ...
  const url = createUrl({
    pathname: `/api/vocabularies/${params.code}`,
    searchParams: params,
  });
  const options: RequestOptions = { ...requestOptions };
