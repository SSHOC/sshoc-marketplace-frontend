...
import { workflowInputSchema } from "@/lib/data/sshoc/validation-schemas/workflow";
...
export function getWorkflowHistory(
  ...
  const url = createUrl({
    pathname: `/api/workflows/${params.persistentId}/history`,
  });
  const options: RequestOptions = { ...requestOptions };
...
export function commitDraftWorkflow(
  ...
  const url = createUrl({
    pathname: `/api/workflows/${params.persistentId}/commit`,
  });
...
export function getWorkflowSources(
  ...
  const url = createUrl({
    pathname: `/api/workflows/${params.persistentId}/sources`,
  });
  const options: RequestOptions = { ...requestOptions };
