...
} from "@/lib/data/sshoc/validation-schemas/media";
...
export function isMediaDetailsFile(
  media: MediaDetails
): media is MediaDetailsFile {
  return "filename" in media;
}

export function isMediaDetailsUrl(
  media: MediaDetails
): media is MediaDetailsUrl {
...
export function createMediaSource(
  ...
  const url = createUrl({ pathname: "/api/media-sources" });
  const json = mediaSourceInputSchema
