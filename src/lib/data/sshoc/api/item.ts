...
import type {
  ...
  PaginatedRequest,
  PaginatedResponse,
} from "@/lib/data/sshoc/api/common";
import type { Dataset, DatasetInput } from "@/lib/data/sshoc/api/dataset";
import type { MediaDetails, MediaDetailsRef } from "@/lib/data/sshoc/api/media";
import type { Property, PropertyInput } from "@/lib/data/sshoc/api/property";
import type {
  Publication,
  PublicationInput,
} from "@/lib/data/sshoc/api/publication";
import type { SourceBase, SourceRef } from "@/lib/data/sshoc/api/source";
...
import type { ConceptBase, ConceptRef } from "@/lib/data/sshoc/api/vocabulary";
import type { Workflow, WorkflowInput } from "@/lib/data/sshoc/api/workflow";
import type {
  WorkflowStep,
  WorkflowStepInput,
} from "@/lib/data/sshoc/api/workflow-step";
import type { RequestOptions } from "@/lib/data/sshoc/lib/client";
import { createUrl, request } from "@/lib/data/sshoc/lib/client";
import type {
  AllowedRequestOptions,
  BooleanString,
  IsoDateString,
  UrlString,
} from "@/lib/data/sshoc/lib/types";
import {
  itemCommentInputSchema,
  itemRelationInputSchema,
  itemRelationKindInputSchema,
  itemSourceInputSchema,
} from "@/lib/data/sshoc/validation-schemas/item";
...
export function isItemDraftSortOrder(
  sortOrder: string
): sortOrder is ItemDraftSortOrder {
  return itemDraftSortOrders.includes(sortOrder as ItemDraftSortOrder);
}

export const itemFacets = [
  "activity",
  "keyword",
  "source",
  "language",
...
export const itemCategoriesWithWorkflowStep =
  enumerate<ItemCategoryWithWorkflowStep>()(...itemCategories, "step");
...
export function isItemCategoryWithWorkflowStep(
  ...
  return itemCategoriesWithWorkflowStep.includes(
    category as ItemCategoryWithWorkflowStep
  );
}
...
type ItemDiffData<T extends object> = {
  [K in keyof T as T[K] extends Array<unknown> ? K : never]: T[K] extends Array<
    infer V
  >
    ? Array<V | null>
    : never;
} & {
  [K in keyof T as T[K] extends Array<unknown> ? never : K]?:
    | T[K]
    | "unaltered";
};
...
export namespace GetItemRelations {
  ...
  export type Params = SearchParams;
  export type Response = PaginatedResponse<{
    itemRelations: Array<ItemRelation>;
  }>;
}
...
export function createItemRelationKind(
  ...
  const json = itemRelationKindInputSchema
...
export function updateItemComment(
  ...
  const url = createUrl({
    pathname: `/api/items/${params.itemId}/comments/${params.id}`,
  });
  const json = itemCommentInputSchema.parse(data);
  const options: RequestOptions = { ...requestOptions, method: "put", json };
...
export function getLastItemComments(
  ...
  const url = createUrl({
    pathname: `/api/items/${params.itemId}/last-comments`,
  });
  const options: RequestOptions = { ...requestOptions };
...
export function createItemSource(
  ...
  const url = createUrl({ pathname: "/api/item-sources" });
  const json = itemSourceInputSchema
    .and(z.object({ code: z.string() }))
    .parse(data);
  const options: RequestOptions = { ...requestOptions, method: "post", json };
...
export function getContributedItems(
  ...
  const url = createUrl({
    pathname: "/api/contributed-items",
    searchParams: params,
  });
  const options: RequestOptions = { ...requestOptions };
