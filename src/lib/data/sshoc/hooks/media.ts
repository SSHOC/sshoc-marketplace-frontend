...
export function useMediaDownload<TData = DownloadMedia.Response>(
  ...
  options?: UseQueryOptions<
    ...
    Error,
    TData,
    ReturnType<typeof keys.download>
  >
) {
  const session = useSession(auth);
  return useQuery(
 ...
 */
export function useMediaThumbnailDownload<
  TData = DownloadMediaThumbnail.Response
>(
  params: DownloadMediaThumbnail.Params,
  auth?: AuthData | undefined,
  options?: UseQueryOptions<
    ...
    },
    {
      ...
      onSuccess(source, ...args) {
        ...
        queryClient.invalidateQueries(
          keys.source.detail({ code: source.code })
        );
  ...
  return useMutation(
    ...
    },
    {
      ...
      onSuccess(...args) {
        queryClient.invalidateQueries(keys.source.lists());
        queryClient.invalidateQueries(
          keys.source.detail({ code: params.code })
        );
        options?.onSuccess?.(...args);
      },
    }
