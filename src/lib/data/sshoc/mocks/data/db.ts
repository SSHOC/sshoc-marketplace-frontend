...
function getRandomElement<T>(map: Map<number | string, T>): T {
  const id = helpers.arrayElement(Array.from(map.keys()));
    ...
    const _itemRelation: ItemRelation = {
      ...
      inverseOf:
        itemRelationStore.size > 0
          ? optional(itemRelation.get().code)
          : undefined,
    };
...
const facets = {
  ...
  addProperty(property?: Property): void {
    ...
    const value = isPropertyConcept(property)
...
const item = {
  ...
  generate(
    ...
    const _item: Omit<ItemBase, "category"> = {
      ...
      status: helpers.arrayElement(itemStatus),
      ...
      properties: times(number.int(12)).map(() => {
        ...
        if (_type.type === "concept") {
          return {
            type: _type,
            concept: helpers.arrayElement(
              vocabulary.get().conceptResults.concepts
            ),
          };
        }
        return {
  ...
  find(
    ...
    const _matches = isNonEmptyString(q)
      ? matchSorter(items, q, { keys: ["label"] })
      : items;
...
function generateJwt(username: string) {
  return new jwt.UnsecuredJWT({ sub: username })
    .setIssuedAt()
