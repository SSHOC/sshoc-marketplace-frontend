...
import type { ItemCategory } from "@/lib/data/sshoc/api/item";
import { useItemCategories } from "@/lib/data/sshoc/hooks/item";
...
export function ItemSearchForm(props: ItemSearchFormProps): JSX.Element {
  ...
  const [itemSearchTerm, setItemSearchTerm] = useState(
    initialItemSearchTerm ?? ""
  );

  function onSubmit(event: FormEvent<HTMLFormElement>) {
    const categories =
      selectedItemCategory !== allItemCategories
...
function useItemCategoryOptions() {
  ...
  const options: ItemCategoryOptions = useMemo(() => {
    const options: ItemCategoryOptions = [
      {
        id: allItemCategories,
        label: t(["common", "home", "search", "all-item-categories"]),
      },
    ];
          ...
          <Item textValue={item.label}>
            ...
            >
              <span aria-hidden style={{ aspectRatio: "1" }}>
                {item.id !== allItemCategories ? (
                  <ItemCategoryIcon category={item.id} />
                ) : null}
              </span>
              {item.label}
            </span>
