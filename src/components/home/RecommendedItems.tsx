...
import { useItemCategories, useItemSearch } from "@/lib/data/sshoc/hooks/item";
    ...
    <section className={css["container"]}>
      ...
      <ul role="list" className={css["sections"]}>
        {keys(itemCategories.data).map((category) => {
          ...
          return (
            <RecommendedItemsForCategory key={category} category={category} />
          );
        })}
      </ul>
    </section>
...
function RecommendedItemsForCategory(
  props: RecommendedItemsForCategoryProps
): JSX.Element {
