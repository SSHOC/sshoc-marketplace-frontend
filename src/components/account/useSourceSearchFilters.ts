...
import type { GetSources } from "@/lib/data/sshoc/api/source";
import { isSourceSortOrder } from "@/lib/data/sshoc/api/source";
...
export function useSourceSearchFilters(): Required<SearchFilters> {
  ...
  const searchFilters = useMemo(() => {
    ...
    const order =
      _order != null && isSourceSortOrder(_order)
        ? _order
        : defaultSourceSortOrder;
