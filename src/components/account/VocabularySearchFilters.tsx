...
import type { ConceptStatus } from "@/lib/data/sshoc/utils/concept";
import {
  conceptStatus,
  mapConceptStatusToFacet,
} from "@/lib/data/sshoc/utils/concept";
    ...
    <aside className={css["container"]}>
      <header className={css["section-header"]}>
        ...
        <div className={css["clear-link"]}>
          <Link href={routes.VocabulariesPage()}>
...
function ConceptStatusFacets(): JSX.Element {
  ...
  const items = conceptStatus.map((status) => {
    return {
      id: status,
      label: t(["authenticated", "concepts", "concept-status", status]),
    };
  });
  ...
  function onChange(keys: Array<string>) {
    searchVocabularies({
      ...filters,
      page: 1,
      [name]: keys as Array<ConceptStatus>,
    });
  }
  ...
  return (
    <div>
      ...
      >
        {items.map((item) => {
          const count =
            searchResults.data?.facets[facet]?.[
              mapConceptStatusToFacet(item.id)
            ]?.count;
