...
import type { ItemSearch, ItemStatus } from "@/lib/data/sshoc/api/item";
import { isItemCategory, isItemStatus } from "@/lib/data/sshoc/api/item";
...
import { isCurationFlag } from "@/lib/data/sshoc/utils/curation";
import { useSearchParams } from "@/lib/core/navigation/useSearchParams";
import { toPositiveInteger } from "@/lib/utils";
import {
  defaultModerateItemsSortOrder,
  isModerateItemSortOrder,
...
export function useModerateItemsSearchFilters(): Required<SearchFilters> {
  ...
  const searchFilters = useMemo(() => {
    ...
    const lastInfoUpdate = searchParams
      .getAll("d.lastInfoUpdate")
      .map((date) => {
    ...
    const searchFilters: Required<SearchFilters> = {
      ...
      "d.conflict-at-source":
        searchParams.get("d.conflict-at-source")?.toLowerCase() === "true",
    };
