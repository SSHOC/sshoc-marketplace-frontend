...
import type { GetDraftItems } from "@/lib/data/sshoc/api/item";
import { isItemDraftSortOrder } from "@/lib/data/sshoc/api/item";
...
export function useDraftItemsSearchFilters(): Required<SearchFilters> {
  ...
  const searchFilters = useMemo(() => {
    ...
    const order =
      _order != null && isItemDraftSortOrder(_order)
        ? _order
        : defaultDraftItemSortOrder;
