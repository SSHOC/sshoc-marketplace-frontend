...
import type { GetUsers } from "@/lib/data/sshoc/api/user";
import { isUserSortOrder } from "@/lib/data/sshoc/api/user";
...
export function useUserSearchFilters(): Required<SearchFilters> {
  ...
  const searchFilters = useMemo(() => {
    ...
    const order =
      _order != null && isUserSortOrder(_order) ? _order : defaultUserSortOrder;
