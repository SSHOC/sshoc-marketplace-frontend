...
import type { GetSources, SourceInput } from "@/lib/data/sshoc/api/source";
import {
  useDeleteSource,
  useUpdateSource,
} from "@/lib/data/sshoc/hooks/source";
...
export function SourceSearchResult(
  props: SourceSearchResultProps
): JSX.Element {
  const { source } = props;
  ...
  const deleteSource = useDeleteSource({ id: source.id }, undefined, {
    meta: deleteSourceMeta,
  });
  ...
  const updateSource = useUpdateSource({ id: source.id }, undefined, {
    meta: updateSourceMeta,
  });
    ...
    <Fragment>
      <SearchResult>
        ...
        <SearchResultContent>
          <MetadataValue>
            <MetadataLabel>
              {t(["authenticated", "sources", "source-url"])}:
            </MetadataLabel>
