import type { ItemCategoryWithWorkflowStep } from "@/lib/data/sshoc/api/item";
...
export function ItemCategoryIcon(props: ItemCategoryIconProps): JSX.Element {
  ...
  switch (props.category) {
    case "dataset":
      return (
        <DatasetIcon width={40} height={40} aria-label={label} title={label} />
      );
    case "publication":
      return (
        <PublicationIcon
    ...
    case "tool-or-service":
      return (
        <ToolIcon width={40} height={40} aria-label={label} title={label} />
      );
    ...
    case "workflow":
      return (
        <WorkflowIcon width={40} height={40} aria-label={label} title={label} />
      );
    case "step":
      return (
        <WorkflowStepIcon
