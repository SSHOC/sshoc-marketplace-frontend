...
import { useActorSearchInfinite } from "@/lib/data/sshoc/hooks/actor";
...
export function ActorComboBox(props: ActorComboBoxProps): JSX.Element {
  ...
  const initialValue = useFieldState<Actor | undefined>(field._root).input
    .value;
  const initialSearchTerm = initialValue?.[labelField] ?? "";

  const [actorSearchTerm, setActorSearchTerm] =
