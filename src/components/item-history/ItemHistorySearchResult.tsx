...
import type { ItemCategory, ItemHistoryEntry } from "@/lib/data/sshoc/api/item";
import { useRevertDatasetToVersion } from "@/lib/data/sshoc/hooks/dataset";
import { useRevertPublicationToVersion } from "@/lib/data/sshoc/hooks/publication";
import { useRevertToolToVersion } from "@/lib/data/sshoc/hooks/tool-or-service";
import { useRevertTrainingMaterialToVersion } from "@/lib/data/sshoc/hooks/training-material";
import { useRevertWorkflowToVersion } from "@/lib/data/sshoc/hooks/workflow";
...
export function ItemHistorySearchResult(
  props: ItemHistorySearchResultProps
): JSX.Element {
  const { item } = props;
    ...
    <SearchResult>
      ...
      <SearchResultMeta>
        <MetadataValue size="sm">
          <MetadataLabel size="sm">
            {t(["common", "item", "last-info-update"])}:
          </MetadataLabel>
      ...
      <SearchResultContent>
        <MetadataValues>
          ...
          <MetadataValue>
            <MetadataLabel>
              {t(["common", "item", "content-contributors", "one"])}:
            </MetadataLabel>
      ...
      <SearchResultControls>
        {item.status === "approved" ? (
          <Link
            href={itemRoutes.ItemEditPage(item.category)({
              persistentId: item.persistentId,
            })}
          >
...
function RevertButton(props: RevertButtonProps): JSX.Element {
  ...
  return (
    <ButtonLink onPress={onRevert}>
      {t(["authenticated", "controls", "revert"])}
    </ButtonLink>
  );
}
