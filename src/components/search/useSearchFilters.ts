...
import type { ItemSearch } from "@/lib/data/sshoc/api/item";
import { isItemCategory, isItemSortOrder } from "@/lib/data/sshoc/api/item";
...
export function useSearchFilters(): Required<SearchFilters> {
  ...
  const searchFilters = useMemo(() => {
    ...
    const order =
      _order.length > 0 ? _order : [defaultItemSearchResultsSortOrder];
