...
import type { Item, ItemCategory } from "@/lib/data/sshoc/api/item";
import { useDataset, useDeleteDataset } from "@/lib/data/sshoc/hooks/dataset";
import {
  useDeletePublication,
  usePublication,
} from "@/lib/data/sshoc/hooks/publication";
import { useDeleteTool, useTool } from "@/lib/data/sshoc/hooks/tool-or-service";
import {
...
function EditItemMenuButton(props: EditItemMenuButtonProps): JSX.Element {
  ...
  const suggested = useItem(category)(
    { persistentId, approved: false },
    undefined,
    { meta }
  );
  const draft = useItem(category)({ persistentId, draft: true }, undefined, {
    meta,
  });
...
function DeleteItemButton(props: DeleteItemButtonProps): JSX.Element {
  ...
  const meta: MutationMetadata = {
    messages: {
      mutate() {
        return t(["common", "controls", "delete-item-pending"], {
          values: { category: label },
        });
      },
      success() {
        return t(["common", "controls", "delete-item-success"], {
          values: { category: label },
        });
      },
      error() {
        return t(["common", "controls", "delete-item-error"], {
          values: { category: label },
        });
      },
    },
