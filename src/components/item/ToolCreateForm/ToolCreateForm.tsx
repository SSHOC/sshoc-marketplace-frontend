...
import { useCreateTool, useUpdateTool } from '@/api/sshoc'
...
export function ItemForm(props: ItemFormProps<ItemFormValues>): JSX.Element {
  ...
  const useItemMutation = persistentIdFromSavedDraft != null ? useUpdateTool : useCreateTool
  ...
  async function onSubmit({ draft, ...unsanitized }: ItemFormValues) {
    ...
    if (persistentIdFromSavedDraft == null) {
      const mutateAsync = create.mutateAsync as ReturnType<typeof useCreateTool>['mutateAsync']

      await mutateAsync([{ draft }, values, { token: auth.session.accessToken }])
    } else {
      const mutateAsync = create.mutateAsync as ReturnType<typeof useUpdateTool>['mutateAsync']
