...
export function ItemProperty(props: ItemPropertyProps): JSX.Element {
  ...
  const [conceptSearchTerm, setConceptSearchTerm] =
  ...
  function onSelectionChange(key: Key | null) {
    const propertyType =
      key != null ? propertyTypesMap.get(key as string) : undefined;
    form.batch(() => {
      /** Store property type `type`, so we can use it in validation schema for `value`. */
      form.change(
        [fieldGroup.type._root, "type"].join("."),
        propertyType?.type
      );
  ...
  return (
    <Fragment>
      ...
      />
      {selectedPropertyType == null ? null : selectedPropertyType.type ===
        "concept" ? (
        <ConceptComboBox
