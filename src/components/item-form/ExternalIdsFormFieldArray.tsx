...
import type { ItemExternalIdInput } from "@/lib/data/sshoc/api/item";
import { useItemSources } from "@/lib/data/sshoc/hooks/item";
...
export function ExternalIdsFormFieldArray(
  props: ExternalIdsFormFieldArrayProps
): JSX.Element {
  const { field } = props;

  const { t } = useI18n<"authenticated" | "common">();
  const fieldArray = useFieldArray<
    ItemExternalIdInput | UndefinedLeaves<ItemExternalIdInput>
...
export function ItemIdentifierField(
  props: ItemIdentifierFieldProps
): JSX.Element {
  ...
  const itemSources = useItemSources(undefined, {
  ...
  const description = (
    <Fragment>
      ...
      {template != null ? (
        <span>
          {" "}
          {t(["authenticated", "itemExternalId", "description"], {
            values: { template },
          })}
        </span>
      ) : null}
