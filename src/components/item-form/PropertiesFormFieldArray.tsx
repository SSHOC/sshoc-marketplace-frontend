...
import { usePropertyTypes } from "@/lib/data/sshoc/hooks/property";
import { usePublishPermission } from "@/lib/data/sshoc/utils/usePublishPermission";
...
export function PropertiesFormFieldArray(
  props: PropertiesFormFieldArrayProps
): JSX.Element {
  const { field } = props;
  ...
  const fieldArray = useFieldArray<
    PropertyInput | UndefinedLeaves<PropertyInput>
  >(field.name, {
    subscription: {},
  });
...
function ItemPropertyAccessControl(
  props: ItemPropertyAccessControlProps
): JSX.Element {
  const { children, name, propertyTypesMap } = props;

  const hasPublishPermission = usePublishPermission();

  const propertyTypeId = useFieldState<PropertyType["code"] | undefined>(name)
    .input.value;
  const propertyType =
    propertyTypeId != null ? propertyTypesMap.get(propertyTypeId) : null;
