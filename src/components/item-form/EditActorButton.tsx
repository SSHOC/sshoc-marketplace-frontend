...
import type { Actor, ActorRef } from "@/lib/data/sshoc/api/actor";
import { useActor, useUpdateActor } from "@/lib/data/sshoc/hooks/actor";
...
export function EditActorButton(props: EditActorButtonProps): JSX.Element {
  ...
  const actorId = useFieldState<ActorRef["id"] | undefined>(field.name).input
    .value;
  ...
  const actor = useActor({ id: actorId! }, undefined, {
...
function EditActorDialogTrigger(
  props: EditActorDialogTriggerProps
): JSX.Element {
