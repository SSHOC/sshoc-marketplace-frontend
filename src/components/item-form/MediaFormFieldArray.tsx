...
import type { ItemMedia, ItemMediaInput } from "@/lib/data/sshoc/api/item";
import { useMediaDownload } from "@/lib/data/sshoc/hooks/media";
...
export function MediaFormFieldArray(
  props: MediaFormFieldArrayProps
): JSX.Element {
  const { field } = props;

  const { t } = useI18n<"authenticated" | "common">();
  const fieldArray = useFieldArray<
    ItemMediaInput | UndefinedLeaves<ItemMediaInput>
  >(field.name, {
    subscription: {},
  });
...
function MediaEditButton(props: MediaEditButtonProps): JSX.Element {
  ...
  const initialValues = useMemo<
    Partial<MediaUploadFormValues> | undefined
  >(() => {
