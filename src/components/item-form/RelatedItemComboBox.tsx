...
import type { RelatedItem } from "@/lib/data/sshoc/api/item";
import { useItemSearchInfinite } from "@/lib/data/sshoc/hooks/item";
...
export interface RelatedItemComboBoxProps {
  field: ItemFormFields["fields"]["relatedItems"]["fields"]["item"] & {
    _root: string;
  };
}

export function RelatedItemComboBox(
  props: RelatedItemComboBoxProps
): JSX.Element {
  const { field } = props;
  ...
  const initialValue = useFieldState<RelatedItem | undefined>(field._root).input
    .value;
  const initialSearchTerm = initialValue?.[labelField] ?? "";

  const [itemSearchTerm, setItemSearchTerm] =
