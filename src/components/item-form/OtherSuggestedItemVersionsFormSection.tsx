...
export function OtherSuggestedItemVersionsFormSection(): JSX.Element {
  ...
  return (
    <FormSection>
      <FormSectionTitle>{t('forms.other-suggested-versions-section')}</FormSectionTitle>
      <p>
        {t(`other-suggested-versions.message.${items.data.length === 1 ? 'one' : 'other'}`, {
            ...
            <li key={[item.persistentId, item.id].join('+')}>
              <article className={css['item']}>
                ...
                <small className={css['item-info']}>
                  {t.rich('other-suggested-versions.suggested-by', {
      ...
      <p>{t('other-suggested-versions.warning')}</p>
    </FormSection>
  )
}
