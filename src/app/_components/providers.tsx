...
export function ContextProviders(props: ContextProvidersProps): JSX.Element {
  const [queryClient] = useState(() => {
    return createQueryClient({
      query: {
        error:
          props.pageProps.dictionaries?.common?.[
            "default-query-error-message"
          ] ?? "Something went wrong.",
      },
      mutation: {
        mutate:
          props.pageProps.dictionaries?.common?.[
            "default-mutation-pending-message"
          ] ?? "Submitting...",
        success:
          props.pageProps.dictionaries?.common?.[
            "default-mutation-success-message"
          ] ?? "Successfully submitted.",
        error:
          props.pageProps.dictionaries?.common?.[
            "default-mutation-error-message"
          ] ?? "Something went wrong.",
      },
    });
  });
