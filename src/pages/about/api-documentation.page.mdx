...
<ApiEndpoint title="Retrieve items by type" endpoint={{ key: 'item-list' }}>

  <ApiParamSelect label="item-type">
    <Item key={1}>/api/tools-services</Item>
    <Item key={2}>/api/training-materials/</Item>
    <Item key={3}>/api/datasets</Item>
    <Item key={4}>/api/publications</Item>
    <Item key={5}>/api/workflows/</Item>
  </ApiParamSelect>
...
  <ApiParamSelect label="allowed property codes" param="d">

<Item key={1}>source</Item>

<Item key={2}>contributor</Item>

<Item key={3}>license</Item>

 <Item key={4}>journal</Item>

<Item key={5}>resource-category</Item>

  </ApiParamSelect>

  <ApiParamTextField label="Search" param="d" placeholder="Search term" />
...
  <ApiParamSelect label="allowed property codes" param="f">

<Item key={1}>activity</Item>

<Item key={2}>keyword</Item>

<Item key={3}>language</Item>

   </ApiParamSelect>

  <ApiParamTextField label="Search" param="f" placeholder="Search term" />
...
- **Contributor suggesting a new item**. After proper authentication (see section above), a contributor can suggest a new item using the token. The example below describes the creation of a new tool. The API call is the following: POST api/tools-services
...
- **Contributor suggesting changes to an existing item**. After proper authentication (see section above), a contributor can suggest changes to an existing item using the token. The example below describes the addition of a new property to an existing training material. The API call to use is: PUT api/training-materials/{persistentId} 
